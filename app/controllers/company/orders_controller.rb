class Company::OrdersController < Company::BaseController
  ...
  def collections_summary
    ...
    @delivered_orders = Order.find_by_date(@delivery_date).includes(:delivery_executive).includes(:debit).includes(:line_items).includes(:user).where.not(state: ['pending','cancelled']).where(category: @category)
  end
